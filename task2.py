...
class Hero(HeroBase, CreatedUpdatedAt, table=True):
    __tablename__ = "users"
...
@app.post("/users/", response_model=HeroPublic)
def create_user(hero: HeroCreate, session: SessionDep):
...
@app.get("/users/", response_model=list[HeroPublic])
...
@app.get("/users/{user_id}", response_model=HeroPublic)
def read_user(user_id: uuid.UUID, session: SessionDep):
    hero = session.get(Hero, user_id)
    if not hero:
        raise HTTPException(status_code=404, detail="User not found")
    return hero


@app.patch("/users/{user_id}", response_model=HeroPublic)
def update_user(user_id: uuid.UUID, hero: HeroUpdate, session: SessionDep):
    hero_db = session.get(Hero, user_id)
    if not hero_db:
        raise HTTPException(status_code=404, detail="User not found")
